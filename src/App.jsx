...
const initialTasks = [
  {
    title: 'Pack the therapy bag',
    note: 'Before 15:30 today',
    source: 'Routine',
    tone: 'lilac',
    done: false,
  },
  {
    title: 'Order diapers',
    note: 'Stock is running low',
    source: 'Email',
    tone: 'coral',
    done: false,
  },
  {
    title: 'Reply to kindergarten email',
    note: 'Two-minute task',
    source: 'Email',
    tone: 'sun',
    done: false,
  },
  {
    title: 'Book Jonas’s pediatrician visit',
...
const alarmUnits = [...reminderUnits, 'at event time']

const demoEvents = initialEvents.map((event) => {
  const settings = {
    'event-1': {
      reminders: [{ value: 15, unit: 'minutes before' }],
      alarm: { enabled: false },
    },
    'event-2': {
      reminders: [{ value: 1, unit: 'hours before' }],
      alarm: {
        enabled: true,
        value: 0,
        unit: 'at event time',
    ...
    'event-3': {
      reminders: [
        { value: 1, unit: 'days before' },
        { value: 2, unit: 'hours before' },
      ],
      alarm: {
        enabled: true,
        value: 10,
    ...
    'event-6': {
      reminders: [{ value: 1, unit: 'days before' }],
      alarm: { enabled: false },
    },
  }
...
const assistantAnswers = {
  'What should I focus on today?':
    'Start with the therapy bag before 15:30. Then your two small tasks can fit into a calm 15-minute window after dinner.',
  'What do I need to prepare for tomorrow?':
    'Tomorrow is light. The helpful prep is to check the diaper supply tonight and keep Jonas’s Vitamin D by the breakfast table.',
  'Are there any conflicts this week?':
    'No conflicts found in this demo. Thursday has both kindergarten errands and speech therapy, so I would prepare the therapy bag on Wednesday evening.',
}
...
function processBrainDump(text) {
  ...
  lines.forEach((line) => {
    ...
    const detail =
      lower.match(
        /(?:monday|tuesday|wednesday|thursday|friday|saturday|sunday|tomorrow|next week|at \d{1,2}(?::\d{2})?)/
      )?.[0] || 'When should this happen?'

    if (
      /(appointment|therapy|doctor|pediatrician|dentist|meeting|visit)/.test(
        lower
      )
    ) {
    ...
    if (
      /(remind|remember|bring|pack|buy|order|pick up|call|reply|send)/.test(
        lower
      )
    ) {
      result.Tasks.push({
    ...
    if (/(vitamin|medication|medicine|dose|tablet|prescription)/.test(lower)) {
      result.Medications.push({
        title: line,
        detail: 'Check timing and dosage',
        icon: '💛',
      })
    }

    if (
      /(every |daily|routine|morning|evening|bedtime|after breakfast)/.test(
        lower
      )
    ) {
      result.Routines.push({
        title: line,
        detail: 'Recurring family rhythm',
        icon: '☁',
      })
    }
  })
  ...
  if (
    !result['Follow-up questions'].length &&
    /(?:need|should|remember)/i.test(text)
  ) {
    result['Follow-up questions'].push({
    ...
    <span className={`event-notification-labels ${compact ? 'compact' : ''}`}>
      ...
      {hasAlarm && (
        <span className="alarm-label">
          ◉ Final Alarm: {formatNotice(event.alarm)}
        </span>
      )}
    </span>
  )
...
function App() {
  ...
  const [chat, setChat] = useState([
    {
      role: 'assistant',
      text: 'Hi — I’m here to make the day feel lighter. What would you like to plan?',
    },
  ])
  ...
  const toggleVoice = () => {
    ...
    if (!SpeechRecognition) {
      setBrainMessage(
        'Voice input is not supported by this browser. You can type your thoughts instead.'
      )
      return
    }
    ...
    recognition.onresult = (event) => {
      ...
      if (transcript) {
        ...
        setBrainMessage(
          'Voice note added. You can keep typing or process it with AI.'
        )
      }
    }
    ...
    recognition.onerror = () => {
      setBrainMessage(
        'Voice input could not capture your speech. Please try again or type your thought.'
      )
    }
  ...
  const handleProcess = () => {
    if (!brainDump.trim()) {
      setBrainMessage(
        'Add a thought first, then I can organize it for you.'
      )
  ...
  const openEditEvent = (event) => {
    setEventForm({
      ...
      alarm: event.alarm || { enabled: false },
    })
  ...
  const deleteEvent = () => {
    if (
      !editingEventId ||
      !window.confirm(
        'Delete this event? This only removes it from the local demo.'
      )
    ) {
      return
    }
    ...
    setEvents((items) =>
      items.filter((item) => item.id !== editingEventId)
    )
    ...
    <main>
      <nav className="topbar">
        ...
        <div className="family-control">
          <button
            ...
            type="button"
            onClick={() => setFamilyMenuOpen((value) => !value)}
            aria-expanded={familyMenuOpen}
          >
            <span className="avatar-stack">
              <i>{selectedMember.firstName.slice(0, 1)}</i>
            </span>
            <span>My Family</span>
            <b>⌄</b>
          </button>
          ...
          {familyMenuOpen && (
            <div className="family-menu">
              {members.map((member) => (
                <div
                  className={`family-menu-row ${
                    member.id === selectedMemberId ? 'active' : ''
                  }`}
                  key={member.id}
                >
                  <button
      ...
      <section id="top" className="hero">
        ...
        <h1>
          Good morning.
          <br />
          <span>Your family is in good hands.</span>
        </h1>
      ...
      <section className="calendar-section">
        ...
        <p className="calendar-note">
          A gentle view of the plans and routines that matter most.
        </p>
      </section>

      <section className="reminders-section">
        <SectionHeading
          eyebrow="A gentle heads-up"
          title="Today’s Reminders & Alarms"
        />
        ...
        <div className="reminder-list">
          <article className="reminder-item">
            <span>◌</span>
            <div>
              <strong>Reminder: Pack therapy bag</strong>
              <small>1 day before</small>
            </div>
          </article>
          ...
          <article className="reminder-item">
            <span>◌</span>
            <div>
              <strong>Reminder: Speech therapy</strong>
              <small>2 hours before</small>
            </div>
          </article>

          <article className="reminder-item alarm-item">
            <span>◉</span>
            <div>
              <strong>Final Alarm: Speech therapy</strong>
        ...
        <p className="notification-note">
          Real push notifications planned for a future version.
        </p>
      </section>

      <section className="tasks-section">
        <SectionHeading
          eyebrow="Small steps, lighter day"
          title="Tasks & to-dos"
          action={
            <span className="count-pill">
              {tasks.filter((task) => !task.done).length} to do
            </span>
          }
        />
        ...
        <div className="task-list">
          {tasks.map((task, index) => (
            ...
            >
              ...
              <span className={`source-badge ${task.tone}`}>
                {task.source}
              </span>
            </button>
          ))}
        ...
        <p className="connection-note">
          Ready for future connections to Todoist, Apple Reminders, Microsoft
          To Do, and Google Tasks.
        </p>
      </section>
      ...
      <section className="signals-section" aria-label="Family signals">
        <article className="overview-card medication-card">
          <div className="card-heading">
            <span className="mini-icon sun">☀</span>
            <h3>Medication</h3>
          </div>

          <button
            className={`medicine-row ${medicationDone ? 'complete' : ''}`}
            onClick={() => setMedicationDone((done) => !done)}
            type="button"
          >
            <span className="check">{medicationDone && '✓'}</span>
            <span>
              <strong>Vitamin D for Child 1</strong>
              <small>After breakfast · daily</small>
            </span>
          </button>
        ...
        <article className="overview-card">
          ...
          <div className="routine-row">
            <time>07:15</time>
            <span>
              <strong>Morning launch</strong>
              <small>Breakfast · bags · jackets</small>
            </span>
          </div>
          ...
          <div className="routine-row">
            ...
            <span>
              ...
              <small>Bath · stories · lights low</small>
            </span>
          </div>
        </article>
        ...
        <article className="overview-card inbox-card">
          <div className="card-heading">
            ...
            <h3>Inbox signals</h3>
            <span className="signal-count">2</span>
          </div>

          <div className="signal">
            <strong>Kindergarten email detected</strong>
            <small>Bring diapers by Thursday</small>
          </div>

          <div className="signal">
            <strong>Therapy update needs review</strong>
            <small>New appointment detail found</small>
          </div>
        </article>
      </section>
      ...
      >
        <div className="brain-dump-intro">
          ...
          <div>
            ...
            <p>
              Type or speak anything on your mind. AI Family OS will organize
              it.
            </p>
          </div>
        </div>
        ...
        <div className={`input-shell ${listening ? 'listening' : ''}`}>
          ...
          <div className="input-actions">
            ...
            >
              ◉ {listening ? 'Stop listening' : 'Voice note'}
            </button>
            ...
            <button
              ...
              disabled={processing}
            >
              ✦ {processing ? 'Finding the threads…' : 'Process with AI'}
            </button>
          </div>
        </div>
      ...
      {processedData && (
        <section className="ai-result">
          <SectionHeading
            eyebrow="A calmer view"
            title="Here’s what I found"
            action={<span className="ai-badge">✦ Mock AI processing</span>}
          />
          ...
          <div className="extracted-grid">
            ...
            <section
        className="assistant-section card"
      ...
      >
        <div className="assistant-intro">
          ...
          <div>
            <p className="eyebrow">Your planning partner</p>
            <h2 id="assistant-title">Ask your family assistant</h2>
            <p>For the questions you don’t want to keep carrying around.</p>
          </div>
        </div>
          ...
          {chat.map((message, index) => (
            ...
            >
              {message.role === 'assistant' && (
                <span className="mini-assistant">✦</span>
              )}
              <p>{message.text}</p>
            </div>
          ))}
        ...
        >
          <input
            ...
            placeholder="Ask about your family’s week…"
            aria-label="Ask the family assistant"
          />
      ...
      <section className="integrations">
        <SectionHeading
          eyebrow="Coming when you’re ready"
        ...
        <div className="integration-grid">
          {[
            ...
            <article className="integration-card" key={name}>
              <span className="integration-logo">{symbol}</span>
              <h3>{name}</h3>
              <small>Planned</small>
            </article>
          ))}
      ...
      {calendarOpen && (
        ...
        >
          ...
          <div className="month-grid">
            ...
            <div className="month-days">
              {monthCells().map((date, index) => {
                if (!date) {
                  return <span className="blank-day" key={`blank-${index}`} />
                }
                  ...
                  >
                    ...
                    <span className="event-dots">
                      {dayEvents.slice(0, 4).map((event) => (
                        <i
                          className={categoryStyles[event.category]}
                          key={event.id}
                        />
                      ))}
                    </span>
          ...
          <p className="calendar-instruction">
            Tap any day to see its plans and reminders.
          </p>
        </Modal>
      )}
      ...
      {selectedDay && (
        <Modal
          label={`Plans for ${formatDate(selectedDay)}`}
          className="day-modal"
            ...
            {events.filter((event) => event.date === selectedDay).length ? (
              events
                ...
                .map((event) => (
                  ...
                  >
                    <span
                      className={`event-dot ${categoryStyles[event.category]}`}
                    />
              ...
              <p className="empty-day">
                Nothing planned yet. Enjoy the breathing room.
              </p>
            )}
          </div>
      ...
      {eventModalOpen && (
        <Modal
          label={editingEventId ? 'Edit event' : 'Add event'}
          onClose={() => {
            setEventModalOpen(false)
            setEditingEventId(null)
          }}
        >
          ...
          <form className="form-stack" onSubmit={saveEvent}>
            ...
            <fieldset className="notification-settings">
              ...
              <p>
                Reminders help you prepare. The final alarm is the last alert
                when it is time to act.
              </p>

              {eventForm.reminders.map((reminder, index) => (
                <div className="custom-reminder" key={index}>
                  <input
                    min="1"
                    type="number"
                    ...
                    onChange={(event) =>
                      setEventForm((value) => ({
                        ...value,
                        reminders: value.reminders.map((item, itemIndex) =>
                          itemIndex === index
                            ? {
                                ...item,
            ...
            <fieldset className="notification-settings">
              ...
              <label className="notification-choice">
                <input
                  type="checkbox"
                  checked={eventForm.alarm.enabled}
                  onChange={(event) =>
                    setEventForm({
                      ...eventForm,
              ...
              {eventForm.alarm.enabled && (
                <div className="custom-reminder">
                  <input
                    min="0"
                    disabled={eventForm.alarm.unit === 'at event time'}
                    type="number"
                    value={eventForm.alarm.value}
                    onChange={(event) =>
                      setEventForm({
                        ...eventForm,
                        alarm: {
                    ...
                    onChange={(event) =>
                      setEventForm({
                        ...
                        alarm: {
                          ...
                          value:
                            event.target.value === 'at event time'
                              ? 0
                              : eventForm.alarm.value,
                        },
                      })
                    }
                  >
      ...
      <footer>
        <span className="brand-mark">✦</span> AI Family OS{' '}
        <i>Capture everything. Organize nothing. AI does the rest.</i>
      </footer>
    </main>
  )
}
